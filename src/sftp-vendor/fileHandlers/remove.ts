...
// deletes the file both locally and on the currently active remote profile
export const removeLocalAndRemote = createFileHandler<FileHandleOption & { skipDir?: boolean }>({
  name: 'removeLocalAndRemote',
  async handle(option) {
    const { localFsPath } = this.target;
    try {
      await fse.remove(localFsPath);
    } catch (error) {
      logger.warn(error.message, `remove local file ${localFsPath} failed`);
    }
    await removeRemote(this, option);
  },
  transformOption() {
    const config = this.config;
    return {
